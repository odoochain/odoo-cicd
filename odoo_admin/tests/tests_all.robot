...
Test Run Unittest
    ...
    ${testruns}=    Odoo Search
    ...    cicd.test.run
    ...    domain=[['branch_id', '=', ${main_branch}]]
    ...    count=True
    ...
    Wait Testruns Done
    ${testrun_id}=    Odoo Search
    ...    cicd.test.run
    ...    domain=[['branch_id', '=', ${main_branch}]]
    ...    limit=1
    ...    order=id desc
    ${testrun_state}=    Odoo Read Field
    ...    cicd.test.run
    ...    ${testrun_id}
    ...    state
    IF    "${testrun_state}" != "failed"    FAIL    testrun should be failed
    Remove File    /opt/src/failtest
