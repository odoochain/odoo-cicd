...
class NewBranch(models.TransientModel):
    ...
    source_branch_id = fields.Many2one(
        "cicd.git.branch", string="Clone From", required=True
    )
    new_name = fields.Char("New Name", required=True)
    dump_id = fields.Many2one("cicd.dump", string="Dump")
    dump_date_modified = fields.Datetime(related="dump_id.date_modified")
    ...
    def default_get(self, fields):
        ...
        if res.get("source_branch_id"):
            source_branch = self.env["cicd.git.branch"].browse(res["source_branch_id"])
            if source_branch.dump_id:
                ...
                for (
                    volume
                ) in source_branch.backup_machine_id.sudo().volume_ids.filtered(
                    lambda x: x.ttype == "dumps"
                ):
                    dump_name = volume.name + "/" + source_branch.backup_filename
                    dump = source_branch.backup_machine_id.sudo().dump_ids.filtered(
    ...
    def _make_branch(self, branch):
        ...
        with LogsIOWriter.GET("cicd", "new_branch") as logsio:
            with repo._temp_repo(machine=machine) as repo_path:
                with machine._gitshell(repo, cwd=repo_path, logsio=logsio) as shell:
                    ...
                    shell.X(["git-cicd", "remote", "set-url", "origin", repo.url])
                    shell.X(
                        [
                            "git-cicd",
                            "push",
