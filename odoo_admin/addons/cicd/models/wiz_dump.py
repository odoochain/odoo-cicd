...
class MakeDump(models.TransientModel):
    ...
    ttype = fields.Selection(
        [("backup", "Backup"), ("restore", "Restore")], required=True
    )
    ...
    backup_volume_id = fields.Many2one(
        "cicd.machine.volume",
        domain=("[('machine_id', '=', machine_id), " "('ttype', 'in', ['dumps'])]"),
    )
    filename = fields.Char("Filename")
    dump_id = fields.Many2one(
        "cicd.dump", string="Dump", domain="[('machine_id', '=', machine_id)]"
    )
    ...
    def default_get(self, fields):
        ...
        if res.get("machine_id") and not res.get("backup_volume_id"):
            ...
            if res["ttype"] == "backup":
                volumes = machine.volume_ids.filtered(lambda x: x.ttype in ["dumps"])
                if len(volumes) == 1:
    ...
    def do_dump(self):
        if self.ttype == "backup":
            ...
            self.branch_id._make_task(
                "_dump", volume=self.backup_volume_id.id, filename=self.filename
            )
