        ...
        for rec in self:
            ...
            if rec.approval_state in ["approved", "declined"]:
                ...
                if (
                    ...
                    and self.approver_id == self.author_user_id
                ):
                    raise ValidationError("Approver mustn't be the same like author.")
    ...
    def _check_approver_codereviewer(self):
        for rec in self:
                ...
                if rec[approver] and rec[approver] == rec.author_user_id:
                    if not self.env.user.has_group("cicd.group_override_approve"):
                        raise ValidationError(
                            ("Code Reviewer and approver must not be " "the author.")
                        )
            for branch in rec.branch_ids:
                ...
                if (
                    not branch.enduser_summary
                    and not branch.enduser_summary_ticketsystem
                ):
                    raise ValidationError(
                        (
